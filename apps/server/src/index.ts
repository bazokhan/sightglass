...
app.get("/api/v1/summary", (request, response) => { const range = dateRange(request.query); response.json(store.summary(range.from, range.to, telemetryFilters(request.query))); });
app.get("/api/v1/trend", (request, response) => { const range = dateRange(request.query); response.json(store.trend(range.from, range.to, telemetryFilters(request.query))); });
app.get("/api/v1/occurrences", (request, response) => { const range = dateRange(request.query); const filters = telemetryFilters(request.query); const operation = text(request.query.operation); const status = text(request.query.status); response.json(store.occurrences({ ...range, ...filters, ...(operation ? { operation } : {}), ...(status ? { status } : {}), limit: clamp(request.query.limit, 1, 200, 50), offset: clamp(request.query.offset, 0, 100_000, 0) })); });
...
app.get("/api/v1/database", (request, response) => { const range = dateRange(request.query); response.json(store.databaseRanking(range.from, range.to, telemetryFilters(request.query))); });
app.get("/api/v1/dependencies", (request, response) => { const range = dateRange(request.query); response.json(store.dependencyRanking(range.from, range.to, telemetryFilters(request.query))); });
app.get("/api/v1/usage", (request, response) => { const range = dateRange(request.query); const tenantId = text(request.query.tenantId); response.json(store.usage(range.from, range.to, { ...telemetryFilters(request.query), ...(tenantId ? { tenantId } : {}) })); });
app.get("/api/v1/usage/trend", (request, response) => { const range = dateRange(request.query); response.json(store.usageTrend(range.from, range.to, telemetryFilters(request.query))); });
...
app.get("/api/v1/health", (request, response) => { const range = dateRange(request.query); response.json(store.health(range.from, range.to, telemetryFilters(request.query))); });
...
function telemetryFilters(query: Record<string, unknown>): { service?: string; environment?: string } {
  const service = text(query.service); const environment = text(query.environment);
