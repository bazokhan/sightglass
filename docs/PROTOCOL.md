...
HTTP 202 acknowledges persistence. Occurrence and meter IDs are idempotency keys; duplicate IDs are successful no-ops. Clients may retry safely. Ordinary occurrences use a bounded in-memory queue. Meter events are additionally spooled to individual local files before transport and deleted only after acknowledgement.
