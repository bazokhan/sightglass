...
export function App() {
  ...
  return <div className="shell">
    ...
    <main>
      <header><div><p className="eyebrow">{tab === "Docs" ? "Product guide" : "Application observability"}</p><h1>{tab}</h1></div>{tab === "Docs" ? null : <div className="controls"><select aria-label="Service" value={service} onChange={(event) => { setService(event.target.value); setEnvironment(""); }}><option value="">All services</option>{[...new Set(data?.services.map((item) => item.service) ?? [])].map((item) => <option value={item} key={item}>{item}</option>)}</select><select aria-label="Environment" value={environment} onChange={(event) => setEnvironment(event.target.value)}><option value="">All environments</option>{[...new Set((data?.services ?? []).filter((item) => !service || item.service === service).map((item) => item.environment))].map((item) => <option value={item} key={item}>{item}</option>)}</select><select aria-label="Time range" value={hours} onChange={(event) => setHours(Number(event.target.value))}><option value={1}>Last hour</option><option value={24}>Last 24 hours</option><option value={168}>Last 7 days</option><option value={720}>Last 30 days</option></select><button className="refresh" onClick={refresh} disabled={pending}>{pending ? "Loading…" : "Refresh"}</button></div>}</header>
