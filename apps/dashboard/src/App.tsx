...
function HealthView({ rows }: { rows: Health[] }) { const latest = new Map<string, Health>(); for (const row of rows) latest.set(`${row.service}:${row.environment}`, row); return <section className="panel"><PanelHeading title="Service health" subtitle="Process and host context with restart history for the selected range." />{latest.size ? <div className="health-grid">{[...latest.values()].map((row) => <article className={`health-card ${row.restartCount ? "restarted" : ""}`} key={`${row.service}:${row.environment}`}><div><i /><strong>{row.service}</strong><span>{row.environment} · PID {row.pid}</span></div><dl><dt>Restarts</dt><dd>{row.restartCount ?? 0}</dd><dt>CPU</dt><dd>{row.cpuPercent.toFixed(1)}%</dd><dt>RSS memory</dt><dd>{bytes(row.memoryRssBytes)}</dd><dt>Host free</dt><dd>{row.hostMemoryFreeBytes === undefined ? "—" : bytes(row.hostMemoryFreeBytes)}</dd><dt>Load (1m)</dt><dd>{row.hostLoad1m?.toFixed(2) ?? "—"}</dd><dt>Disk free</dt><dd>{row.diskFreeBytes === undefined ? "—" : bytes(row.diskFreeBytes)}</dd><dt>Event-loop lag</dt><dd>{row.eventLoopLagMs.toFixed(1)} ms</dd><dt>Uptime</dt><dd>{duration(row.uptimeSeconds)}</dd></dl></article>)}</div> : <Empty />}</section>; }
