...
try {
  ...
  const resultsDir = join(root, "docs", "benchmarks");
  const assetsDir = join(root, "docs", "assets", "benchmarks");
  ...
  writeFileSync(join(root, "apps", "dashboard", "src", "benchmark-results.ts"), dashboardModule(result));
  ...
  console.log("Wrote docs/benchmarks/latest.json, docs/benchmarks/LATEST.md, docs/assets/benchmarks/*.svg, and dashboard benchmark data.");
} finally {
...
function dashboardModule(result: unknown): string { return `// Generated by npm run benchmark. Do not edit by hand.\nexport const benchmarkResults = ${JSON.stringify(result, null, 2)} as const;\n`; }
