...
function Content({ tab, data, openOccurrence }: { tab: Tab; data: DashboardData; openOccurrence(id: string): void }) {
  ...
  if (tab === "Database") return <RankingTable title="Database work" subtitle="Normalized Prisma operations ranked by aggregate time." rows={data.database} columns={["operation", "calls", "totalMs", "averageMs", "slowestMs"]} />;
  if (tab === "Dependencies") return <RankingTable title="Outbound dependencies" subtitle="Safe destinations and normalized paths captured inside observed operations." rows={data.dependencies} columns={["host", "method", "path", "calls", "errors", "averageMs", "slowestMs"]} />;
